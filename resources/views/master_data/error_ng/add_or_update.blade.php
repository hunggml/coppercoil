	            ...
	            <div class="card">
	                ...
	                <form role="from" method="post" action="{{route('masterData.error.addOrUpdate')}}">
		                ...
		                <div class="card-body">
		                	<div class="row">               
		                    	<div class="form-group col-md-4 hide">
				                    ...
				                    <input type="text" value="{{old('ID') ? old('ID') : ($error ? $error->ID : '') }}" class="form-control" id="idError" name="ID" readonly>
				                </div>
				                <div class="form-group col-md-4">
				                    <label for="nameError">{{__('Name')}} {{ __('Error') }}</label>
				                    <input type="text" value="{{old('ID') ? old('ID') : ($error ? $error->Name : '') }}" class="form-control" id="nameError" name="Name" placeholder="{{__('Enter')}} {{__('Name')}}" required>
				                </div>
								<div class="form-group col-md-4">
				                    <label for="MaterialsProduct">{{__('Handle')}}</label>
				                    <input type="Text" value="" class="form-control handle" id="Handle" name="Handle" placeholder="{{__('Enter')}}" >
				                </div>
                                ...
                                <div class="form-group col-md-3 quan">
				                    ...
				                    <input type="text" value="{{old('ID') ? old('ID') : ($error ? $error->Note : '') }}" class="form-control" id="NoteError" name="Note" placeholder="{{__('Enter')}} {{__('Note')}}" >
				                </div>
								@if($error)
									@foreach($error->handle as $value)
									<div class="col-3 tr-{{ str_replace(' ','',$value->Handle)}}">
										<label class="mater"> {{__('Handle')}} ID</label>
										<input type="text" name="ListHandle[]" id="idCan66" value="{{$value->Handle}}" class="form-control" readonly>
									</div>
									<div class="col-2 tr-{{ str_replace(' ','',$value->Handle)}} ">
										<button type="button" id="{{ str_replace(' ','',$value->Handle)}}" class="btn btn-danger btn-delete-box" style="margin-top:10%">
												{{__('Delete')}}
										</button>
									</div>
									@endforeach
		                ...
		                <div class="card-footer">
		                	<a href="{{route('masterData.error')}}" class="btn btn-info" style="width: 80px">{{__('Back')}}</a>

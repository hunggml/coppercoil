            ...
            <div class="row">
              ...
              <div class="form-group col-md-2">
                ...
                <select class="custom-select select2" name="Wire_Type">
                  ...
                  <option value="{{$key}}"  {{$request->Wire_Type == $key ? 'selected' : ''}}>
                    {{$key}}
                  </option>
                  @endforeach
                </select>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                ...
                <button  class="btn btn-detail1 btn-info" id="btn-{{$value->Command_ID}}-{{$value->Materials_ID}}-{{$value->Pallet_ID}}"  data-toggle="modal" data-target=".bd-example-modal-lg">
                  {{__('Detail')}}
                </button>
                @if($value->Status == 0)
                @if(Auth::user()->level == 9999 || Auth::user()->checkRole('create_import'))
                <button  class="btn btn-warning btn-add" id="btn-{{$value->Command_ID}}-{{$value->Materials_ID}}-{{$value->Pallet_ID}}"  data-toggle="modal" data-target="#add-stock">
                  {{__('Import')}}
                </button>
                <button class="btn btn-danger btn-cancel" id="btn-{{$value->Command_ID}}-{{$value->Materials_ID}}-{{$value->Pallet_ID}}" >
                  {{__('Cancel')}}
                </button>
                @endif
                @endif
...
<!-- 
        ...
        <form action="{{ route('warehousesystem.import.detail.add_stock') }}" method="post">
          ...
          <div class="modal-body">
            <div class="form-group ">
              ...
              <br>
              <button type="button" class="btn btn-add-stock float-right btn-warning hide">{{__('Import')}}</button>
  ...
  <!-- modal add pallet -->
  <div class="modal fade" id="add-pallet" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="title">{{__('Import')}}</h5>
        ...
        <form action="{{ route('warehousesystem.import.detail.add_pallet') }}" method="post">
          ...
          <div class="modal-body">
            <div class="form-group body-add-pallet ">
              <input type="text" name="Command_ID" value="{{$request->ID}}" id="Command_ID" class="form-control Command_ID hide" >
              <label> {{__('Pallet')}} </label>
              ...
              <br>
              <button type="button" class="btn btn-add-Box float-right btn-warning ">{{__('Create')}}</button>
              <br>
              <br>
            </div>
          </div>
